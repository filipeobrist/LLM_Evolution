# Mais mutation rate, menos steps no primeiro treino, fitness change: use loss instead of f1, and less penalization
import math
import torch
...
BASELINE_LATENCY = 0.008
BASELINE_PARAMS = 40_000_000
    ...
    while step_count < steps:
        for batch in loader:
            ...
            if step_count % val_check_interval == 0 and val_ds is not None:
                current_f1, _, _ = evaluate_model(model, val_ds)
...
def evaluate_model(model, val_ds):
    model.eval()
    # Podes usar batch_size > 1 para ser mais rápido!
    loader = DataLoader(val_ds, batch_size=BATCH_SIZE) 
    all_preds, all_labels, latencies, losses = [], [], [], []
    ...
    for batch in loader:
        ...
        torch.cuda.synchronize() 
        # Latência média por amostra no batch
        latencies.append((time.perf_counter() - start_lat) / input_ids.size(0))
        
        loss = criterion(logits, labels)
        losses.append(loss.item())
        
        # --- A CORREÇÃO AQUI ---
        # Pegamos em todas as predições do batch de uma vez
        preds = torch.argmax(logits, dim=-1) 
        
        # Usamos .tolist() e .extend() em vez de .item() e .append()
...
def evaluate_individual(base_model, genotype, train_ds, val_ds, steps, inherited_weights=None):
    ...
    f1, latency, loss = evaluate_model(model, val_ds)

    print(f"Evaluated Genotype: {genotype} | F1: {f1:.4f} | Latency: {latency:.4f}s  | Loss: {loss:.4f} | Train Time: {train_time:.2f}s")
    ...
    stats = {
        'f1': f1, 'latency': latency, 'loss': loss,
        'params': total_params,
...
def fitness(population_list):
    ...
    for ind in population_list:
        stats = ind['stats']
        val_loss = stats.get('loss', 2.0)
        
        performance_score = math.exp(-val_loss)

        lat_ratio = stats['latency'] / BASELINE_LATENCY
        latency_penalty = min(1.0, math.exp(-0.5 * (lat_ratio - 1.0)))


        param_ratio = stats['params'] / BASELINE_PARAMS
        param_penalty = min(1.0, math.exp(-0.2 * (param_ratio - 1.0)))


        if stats.get('f1', 0) < 0.25:
            ind['fitness'] = 0.0001
        else:
            # Multiplicamos tudo. Se um falhar, a fitness cai.
            ind['fitness'] = performance_score * latency_penalty * param_penalty
...
def evolve(base_model, train_ds, val_ds, pop_size=30, generations=100, elitism=1):
    ...
    for gen in range(generations):
        ...
        pd.DataFrame(history_logs).to_csv("agnews_evolution_results_v3.csv", index=False)
