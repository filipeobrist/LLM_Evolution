...
def run_baseline():
    print(f"--- Avaliando Baseline (Mini-Jamba Vanilla) ---")
    print(f"Config: Batch {BATCH_SIZE}, Steps {STEPS_BASELINE}")
    
    # 1. Carregar Tokenizer e Dados
    tokenizer = AutoTokenizer.from_pretrained("TechxGenus/Mini-Jamba")
    train_ds, val_ds = load_agnews(tokenizer)
    
    # 2. Carregar o Modelo Original (Genótipo padrão do Mini-Jamba)
    # Geralmente o Mini-Jamba tem uma estrutura fixa. 
    # Precisas de saber o genótipo que corresponde ao modelo original.
    # Se o original tem 16 camadas e são todas Jamba (1), o genótipo é [1]*16
    vanilla_genotype = [1] * 16 
    
    base_model = from_pretrained("TechxGenus/Mini-Jamba").to(DEVICE)
    
    # 3. Avaliar do Zero (Sem herança de pesos de runs anteriores)
    print("Iniciando treino do baseline...")
    weights, stats, losses = evaluate_individual(
        base_model, 
        vanilla_genotype, 
        train_ds, 
        val_ds, 
        steps=STEPS_BASELINE, 
        inherited_weights=None # Baseline tem de ser do zero!
    )
    
    print("\n--- RESULTADO BASELINE ---")
    print(f"F1-Score: {stats['f1']:.4f}")
    print(f"Latência: {stats['latency']:.6f} s/sample")
...
# --- RESULTADO BASELINE ---
# F1-Score: 0.8115
# Latência: 0.018901 s/sample
# Parâmetros: 69,459,460
# Profundidade: 16