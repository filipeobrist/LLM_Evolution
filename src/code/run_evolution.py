...
def generate_random_genotype(min_layers=MIN_LAYERS):
    """Generate a random genotype with a random length between min_layers and MAX_LAYERS."""
    length = random.randint(min_layers, MAX_LAYERS)
    return [random.randint(0, 1) for _ in range(length)]

def crossover(g1, g2):
    """Single-point crossover between two genotypes. Returns a new child genotype."""
    ...
    if len(child_g) > MAX_LAYERS:
        ...
        # If too small, add random layers until the minimum is reached
...
def plot_population_vs_best(data):
    """ Plots the average loss curve of the population against the best individual in generation 0.
    """
    ...
    plt.title('Convergence of Population vs Best Individual in Generation 0')
    plt.legend()
...
def evolve(base_model, train_ds, val_ds, pop_size=30, generations=100, elitism=1, run_name="evolution_run"):
    """Main evolution loop. Returns the best individual found across all generations."""
    history_logs = []
    gen0_data = []
    best_overall_f1 = -1.0
    best_overall_data = {}
...
def run_thesis_experiment():
    for run_idx, seed in enumerate(SEEDS):
        ...
        print(f"Run {run_idx+1} concluded. Best F1: {best_of_run['f1']:.4f} (Gen {best_of_run['generation']})")
