...
# ==========================================
CHECKPOINT_NAME = "best_model_run_5_seed_7.pt" # Alterar para ficheiros a usar
OUTPUT_CSV = "trained_model_results_run_5.csv"
MODEL_SAVE_PATH = "trained_model_run_5.pt"
BATCH_SIZE = 32
EPOCHS = 4
LEARNING_RATE = 3e-5

class IndividualModel(nn.Module):
    def __init__(self, base_model, genotype, num_classes=4):
        super(IndividualModel, self).__init__()
        # Usamos o deepcopy para garantir que não sujamos o template original
        self.backbone = copy.deepcopy(base_model)
        self.genotype = genotype
        
        # Filtramos as camadas conforme o genótipo
        # No JambaLM, as camadas estão em self.backbone.jamba.layers
        original_layers = self.backbone.jamba.layers
        new_layers = nn.ModuleList()
        for i, bit in enumerate(self.genotype):
            if bit == 1 and i < len(original_layers):
                new_layers.append(original_layers[i])
        
        # Reatribuímos as camadas filtradas
        self.backbone.jamba.layers = new_layers
    ...
    def forward(self, input_ids, attention_mask=None):
        ...
        # 2. Passagem pelas camadas (Blindagem contra Tuplos)
        for layer in self.backbone.jamba.layers:
            # Garantir que x é tensor antes de entrar na camada
            if isinstance(x, tuple):
                x = x[0]
                
            x = layer(x)
            
            # Garantir que x é tensor depois de sair da camada
            if isinstance(x, tuple):
                x = x[0]
        
        # 3. Normalização Final (Blindagem Extra)
        if isinstance(x, tuple):
            x = x[0]
        x = self.backbone.final_layernorm(x)
        
        # 4. Mean Pooling
        # x shape: [Batch, Seq, Hidden] -> [Batch, Hidden]
...
def evaluate_full(model, loader, device, criterion):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            # Medição rigorosa de latência (apenas inferência)
            start_lat = time.time()
...
def run_intensive_benchmark():
    ...
    # 1. Carregar Dataset AG News (Completo)
